...
def macd(pd_dataframe):
    ...
    portfolio['positions'] = (positions.multiply(goog_data['c'], axis = 0))

    portfolio['cash'] = initial_capital - (positions.diff().multiply(goog_data['c'], axis=0)).cumsum()
    ...
    ax1.plot(goog_data.loc[goog_data.orders== 1.0].index,
            pd_dataframe["c"][goog_data.orders == 1.0],
...
def macd2(df, period1, period2, period3): # default MACD period values are: period1 = 26, period2 = 12, period3 = 9.
    EMA_1 = df['c'].ewm(span=period1, adjust=False).mean()
    EMA_2 = df['c'].ewm(span=period2, adjust=False).mean()
