...
def tr(data):
    data['previous_close'] = data['c'].shift(1)
    data['high-low'] = abs(data['h'] - data['low_price'])
    data['high-pc'] = abs(data['h'] - data['previous_close'])
...
def supertrend(df, period=7, atr_multiplier=2):
    ST = ta.supertrend(df['h'], df['l'], df['c'], 7, 2)
